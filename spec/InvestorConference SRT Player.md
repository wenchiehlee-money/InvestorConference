...
- 字幕：`{stockId}/{stockId}_{year}_q{quarter}.srt`、`{stockId}_{year}_q{quarter}_GT.srt` 或 `{stockId}_{year}_q{quarter}_FIN.srt`
- PDF：`{stockId}/{stockId}_{year}_q{quarter}_{label}.pdf`

GTC / Podcast：
- 音訊：`{folder}/{stem}.mp3|m4a|wav`
- 字幕：`{folder}/{stem}.srt`、`{stem}_GT.srt` 或 `{stem}_FIN.srt`
- PDF：`{folder}/{stem}_{label}.pdf`

### SRT badge 規則

- `GT`：檔名包含 `_GT.srt`
- `FIN`：一般 `.srt` 或檔名包含 `_FIN.srt`
