...
- PDF-to-MD conversion belongs to `skills/skill-mlx-api-client-ocr`; do not call ad hoc OCR clients or hard-coded local Windows paths.
...
If OCR could not finish, the Markdown must retain machine-readable `TODO:OCR` markers so `skills/skill-mlx-api-client-ocr/scripts/refine_todo_ocr.py` can repair only those pages later.
